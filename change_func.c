...
void cge_name_func(DATA *command,int sockfd)
{
	...
	//发送信息结构体
	ret = send(sockfd,command,sizeof(DATA));
	if(ret < 0){
...
void cge_year_func(DATA *command,int sockfd)
{	
	int ret;
	//修改标志位：修改年龄
	command->protocol = LOG_CGE_YEAR;
	//获取修改的年龄
	printf("请输入修改后的年龄：\n");
	scanf("%d\n",&command->data.info.year);
	//发送信息结构体
	ret = send(sockfd,command,sizeof(DATA));
	if(ret < 0){
		perror("cge_year_func send error\n");
		return -1;
	}
	//接收信息结构体
	memset(command,0,sizeof(DATA));
	ret = recv(sockfd,command,sizeof(DATA),0);
	if(ret < 0){
		perror("cge_year_func recv error\n");
		return -1;
	}
	if(command->protocol == CGE_year_OK){
		printf("修改年龄成功！\n");
	}else{
		printf("修改年龄失败！\n");
	}
